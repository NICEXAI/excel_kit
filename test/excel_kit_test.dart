...
void main() {
  test("Read xlsx file", () {
    String readPath = path
        .canonicalize(path.join(path.current, "test/resource/read_test.xlsx"));
    ...
    // get sheet content
    var sheet =
        ExcelKit.readFile(readPath).getSheet("Sheet1", fieldMap: fieldMap);
  ...
  test("Write xlsx file", () {
    String writePath = path
        .canonicalize(path.join(path.current, "test/resource/write_test.xlsx"));

    List<Map> dataList = [
      {"id": 1, "name": "user1", "age": 12},
      {"id": 2, "name": "user2", "age": 15}
    ];
    ...
    ExcelKit.writeFile(
        writePath, [SheetOption("Sheet1", dataList, fieldMap: fieldMap)]);
  });
}
